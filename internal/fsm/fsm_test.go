...
func TestValidateDuplicateTransitionDifferentTarget(t *testing.T) {
	// 同一 (from, event) 指向不同目标：非确定性，必须拒绝。
	d := orderDef()
...
func TestValidateDuplicateTransitionSameTarget(t *testing.T) {
	// Even if the target is the same, a duplicate (from, event) pair must be rejected.
	d := orderDef()
...
func TestApplyUndefinedEvent(t *testing.T) {
	// paid 有 ship/cancel，没有 deliver：未定义转移。
	d := orderDef()
...
func TestTerminals(t *testing.T) {
	d := orderDef()
	got := d.Terminals()
	want := []string{"cancelled", "delivered"} // 按名称升序
...
func TestPathForwardOnly(t *testing.T) {
	// pending->delivered 正向可达，但 delivered->pending 不可达（delivered 是终态）。
	d := orderDef()
...
func TestPathUnreachable(t *testing.T) {
	// delivered 与 cancelled 互不可达（都是终态）。
	d := orderDef()
...
func TestPathShortestAmongAlternatives(t *testing.T) {
	// pending 既能经 cancel 直达 cancelled，也能经 pay->cancel 到达 cancelled。
	// BFS 应返回更短的 [cancel]。
	d := orderDef()
	reach, events := d.Path("pending", "cancelled")
